...
#---------------------------Algoritmo: Sistema de Colonias de Hormigas + BL---------------------
...
HEURISTIC<-function(training,feature){
  ...
  #for(class in 1:nClasses){
   # for(fij in 1:Nfij){
  heuristic<-sapply(seq_along(1:nClasses),function(i){
    sapply(seq_along(1:Nfij),function(j){
      heuristic<-heuristic+((Table[j,i]/(sum(Table[j,])))+log2((Table[j,i]/(sum(Table[j,])))/((sum(Table[,i])/nrow(training))*(sum(Table[j,])/nrow(training)))))
    })
  })
    #  heuristic<-heuristic+((Table[fij,class]/(sum(Table[fij,])))+log2((Table[fij,class]/(sum(Table[fij,])))/((sum(Table[,class])/nrow(training))*(sum(Table[fij,])/nrow(training)))))
   # }
 # }
...
Transition_Rule<-function(AntSol,taoCar,heur){
  alpha<-1
  beta<-2
  q<-sample(0:1,1)
  q0<-0.8
  ...
  if(q<=q0){
    ...
    probs<-sapply(seq_along(tao_car),function(j){
      if(AntSol[j]==0){
      pheromoneDotHeristic[j]/(sum(pheromoneDotHeuristic))
      }else 0
    })
    selectedFeature<-which.max(probs)
  }
  AntSol[selectedFeature]<-1
  return (list(AntSol,selectedFeature))
}

tr<-Transition_Rule(L[[1]],tao_car,heuristics)

library(arules)
myh<-HEURISTIC(training,training[[6]])

SHC-LS<-function(training,test){
  
  library(arules) #load library for discretization
  
  #params initialization
  featuresIndexes<-1:(ncol(training)-1)
  tao_car<-rep(10^-6,(ncol(training)-1),replace=T)
  tao_num_car<-1/featuresIndexes
  nEval<-0
  nAnts<-10
  ...
  while(nEval<15000){
    #construct list of factible candidates. Initially all 0 (all are factible)
    J<-lapply(seq_along(1:nAnts),function(i){
      rep(0,ncol(training))
    })
    
    #selecting nmber of features to select by each ant
      featuresToSelect<-sapply(seq_along(1:nAnts),function(i){
        which(tao_num_car==sample(tao_num_car,1,rep=FALSE))
      })

    
    #construct solutions by Ants
    nc<-featuresToSelect[1]
    t<-1
    for(i in 1:nc){
      for(j in 1:nAnts){
        transition<-Transition_Rule(L[[j]],tao_car,heuristics)
        index<-transition[[2]]
        SordenList[[j]][i]<-index
        L[[j]]<-transition[[1]]
        #update pheromone locally
        tao_car[index]<-((1-phi)*tao_car[index]*phi*(10^-6))
      }
      t<-t+1
      nc<-featuresToSelect[t]
    }
  }
